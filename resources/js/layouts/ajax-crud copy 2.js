...
$(document).on('click', '.btn-update-entity', function(e) {
    ...
    const recordId = $(form).find('[name="beneficiary_id"]').val();/////*************************************************************** */
    if (!recordId) {
...
function fillEditFormRaa(responseData, formSelector) {
    ...
    formB.find('[name="beneficiary_id"]').val(responseData.beneficiary_id);
...
$(document).on('click', '.btn-save-request', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // إظهار رقم الطلب في العنصر <span class="request-number">
            if (response.request_id) {
                $('.request-number').text(response.request_id);
            }

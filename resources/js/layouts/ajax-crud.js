...
$(document).on('click', '.btn-save-entity', function (e) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // تنظيف إضافي للتأكد من إغلاق المودال
                setTimeout(function() {
...
///////////////////////////////////////داله البحث ////////////////////////////////////////////////////////
/*************************************************************** لتعامل من غير الModel مع ارجاع رقم المستفيد ********************************************/
...
///////////////////////////////////////داله التعديل حدود قطعة الارض////////////////////////////////////////////////////////

$(document).on('click', '.btn-update-boundaries', function (e) {
    e.preventDefault();
    ...
    // ✅ الحصول على ID من الفورم (حقل مخفي)
    const recordId = $(form).find('[name="plot_id"]').val();/////*************************************************************** */
    if (!recordId) {
        console.error('لم يتم العثور على id داخل الفورم!');
        return;
    }
...
/*************************************************************** داله الحفظ مع جلب رقم الطلب  مع اخفاء الزر بعد الحفظ********************************************/
...
$(document).on('click', '.btn-save-request-number', function (e) {
    ...
    const modal = button.data('modal');
    const reloadContainer = button.data('reload-container');
    let reloadUrl = button.data('reload-url'); // استخدم let بدل const
    const url = button.data('url');
    const method = button.data('method') || 'POST';
...
/*************************************************************** داله الحفظ مع جلب رقم الطلب بدون تعطيل الزر ********************************************/

$(document).on('click', '.btn-save-request-number-loc', function (e) {
    e.preventDefault();
    const button = $(this);
    const form = $(button.data('form'))[0];
    const modal = button.data('modal');
...
/*************************************************************** داله الحفظ مع جلب رقم الطلب وارجاع رقم قطعه الارض الا فورم الحدود********************************************/

$(document).on('click', '.btn-save-land_plots-number', function (e) {
    e.preventDefault();
    const button = $(this);
    const form = $(button.data('form'))[0];
    const modal = button.data('modal');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            Swal.fire({
             ...
             // إظهار رقم القطعه ومعرّفه في العنصر
            let formB = $('#boundariesForm');
             formB.find('[name="plot_id"]').val(response.id);
...
//////////////////////////////////////////  داله الحذف  مع جلب رقم الطلب لكي يعرض بحسب الرخصه////////////////////////////////////////////////////////
...
$(document).on('click', '.btn-delete-component', function (e) {
    ...
    const method = button.data('method') || 'DELETE';
    const reloadContainer = button.data('reload-container');
    let reloadUrl = button.data('reload-url');
...
//////////////////////////////  اكواد اضهار بينات التعديل للفورم  بدون التعامل مع Model ///////////////////////////////////////

function fillEditFormnotmodel(responseData, formSelector) {
    const form = $(formSelector);
...
$(document).on('click', '.edit-record-btn-ont-model', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log(response);
            fillEditFormnotmodel(response, formId);
...
$(document).on('click', '#searchBtnRequests', function () {
    const button = $(this);
    const url = button.data('reload-url');          // الرابط (route) اللي بيجيب الجدول
    const container = $(button.data('reload-container')); // التارغت اللي بيتم تحميل الجدول فيه
    const status = button.data('status');           // 👈 القيمة الثابتة (مثلاً: قيد المراجعة)

    if (!url || !container.length) return;

    // رسالة مؤقتة أثناء تحميل البيانات
    container.html('<tr><td colspan="100%">جاري تحميل البيانات...</td></tr>');
    ...
    $.ajax({
        ...
        url: url,
        data: {
            status: status  // 👈 نرسل القيمة الثابتة
        },
        success: function (response) {
            container.html(response); // استبدال الجدول بالنتيجة
        ...
        error: function (xhr) {
            console.error(xhr.responseText);
            container.html('<tr><td colspan="100%">حدث خطأ أثناء تحميل البيانات</td></tr>');
        }
    });
});


///////////////////////////////////////////////////بلاسم والرقم داله بحث الحالات /////////////////////////////////////////////////////////////

$(document).ready(function() {
    // الحدث عند الكتابة في حقل البحث
    $(document).on('input', '.search-box input', function () {
        const input = $(this);
        const container = $('#RequestsTableBody'); // أو استعمل data-attribute لو متعدد
        const url = input.data('reload-url'); // ضع رابط البحث هنا أو في data-reload-url للحقل

        const request_number = input.val(); // القيمة المدخلة
        ...
        $.ajax({
            type: 'GET',
            url: url,
            data: { request_number: request_number }, // سترسل للـ controller على شكل $_GET['query']
            success: function(response) {
                container.html(response);
            },
            error: function(xhr) {

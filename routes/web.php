...
use App\Http\Controllers\allRquestPlatform;
...
use App\Http\Controllers\PaymentControllerPlatform;
...
////////////////////////////-سامي-///////////////////////////////////////////
use App\Http\Controllers\SimpleCustomReportController;
...
// ------------------------------------------------------
// عرض المنصة (الصفحة الرئيسية) بدون تسجيل دخول


// =======================================
// الصفحة الرئيسية → المنصة (بدون تسجيل دخول)
Route::get('/', function () {
    return view('platform_user.app');
})->name('platform.home');

// راوت التنقل الداخلي عبر AJAX بدون تسجيل دخول
...
Route::middleware(['auth'])->group(function () {
    ...
    // routes/web.php
Route::get('/user/permissions', [Users1Controller::class, 'getUserPermissions'])->name('user.permissions');
Route::get('/user/permissions_details', [Users1Controller::class, 'getUserPermissions'])->name('user.permissions_details');
Route::get('/user/permissions/backup', [Users1Controller::class, 'getUserPermissions'])->name('user.permissions.backup');
...
//////////  انشاء حساب ///////////////////////////
Route::get('/beneficiary/register', [BeneficiaryRegisterController::class, 'showRegisterForm'])
    ->name('beneficiary.register');

Route::post('/beneficiary/register', [BeneficiaryRegisterController::class, 'register'])
    ->name('beneficiary.register.submit');



// =======================================
// روت الملف الشخصي للمستفيد
...
// لوحة المستفيد (المنصة) بعد تسجيل الدخول (اختياري)
Route::middleware(['auth:beneficiary'])->group(function () {
    Route::get('/beneficiary/dashboard', function () {
        ...
        // الدفع
    Route::get('/paymentsPlatform/info/{requestId}', [PaymentControllerPlatform::class, 'info'])->name('paymentsPlatform.info');
    Route::post('/paymentsPlatform/charge', [PaymentControllerPlatform::class, 'charge'])->name('paymentsPlatform.charge');


});

// =======================================
// روابط AJAX داخل المنصة
Route::get('/ajax-page-platform/{page}', [PageControllerPlatform::class, 'loadPage'])
    ->where('page', '.*')
    ->name('ajax.page.platform');
...
Route::post('/add-requestsPlatform_Ajax', [allRquestPlatform::class, 'store'])->name('add-requestsPlatform_Ajax');
...
Route::get('/get_street_type', [DirectorateController::class, 'get_street_type']);
Route::get('/get_governorate', [DirectorateController::class, 'get_governorate']);

Route::get('/directorates_street_type', [DirectorateController::class, 'directorates_street_type']);
Route::get('/get_directorates', [DirectorateController::class, 'get_directorates']);
Route::get('/get_role', [DirectorateController::class, 'get_role']);
...
/////////////////////////////////
Route::get('/reload-Directorate_table',[DirectoratesController::class, 'getUsers'])->name('reload-Directorate_table');
Route::get('/index-Directorate_table',[DirectoratesController::class, 'index'])->name('index-Directorate_table');
...
Route::get('/roles/stats', [RoleController::class, 'statsCards'])->name('roles.stats');
Route::get('/roles/permission-usage', [RoleController::class, 'permissionUsage'])->name('roles.permission-usage');

Route::prefix('roles')->group(function () {
    Route::get('/permissions-management', [RoleController::class, 'index'])->name('roles.permissions.management'); // صفحة إدارة الأدوار والصلاحيات
    ...
    Route::post('/update/{id}', [RoleController::class, 'update'])->name('roles_update'); // تحديث دور
...
Route::get('/requests/submit_Report_s/{id}',[allRquest::class, 'requests_submit_Report'])->name('requests.submit_Report');



/////////ارسال الطلب للمراجعه من لمنصه
Route::get('/requests_platform/submit/{id}',[allRquestPlatform::class, 'requests_submit'])->name('requests.submit');


///      ازرار الاجرائات
Route::prefix('requests')->group(function () {
    Route::get('/{id}/workflow', [RequestController::class, 'getWorkflow'])->name('requests.workflow');// جلب سير العمل
    Route::post('/{id}/resption', [RequestController::class, 'resption'])->name('requests.resption');// قبول من موظف الاستقبال
    ...
    Route::post('/{id}/approve_Change', [RequestController::class, 'approve_Change'])->name('approve.change'); // طلب تعديل
...
// التقارير (صفحة الرسوم البيانية )
Route::prefix('charts-reports')->group(function () {
    Route::get('/', [\App\Http\Controllers\ChartsReportController::class,'index']);
    Route::get('/meta', [\App\Http\Controllers\ChartsReportController::class,'meta']);
    Route::get('/summary', [\App\Http\Controllers\ChartsReportController::class,'summary']);
    Route::get('/charts/employee-performance', [\App\Http\Controllers\ChartsReportController::class,'employeePerformance']);
    Route::get('/charts/region-distribution', [\App\Http\Controllers\ChartsReportController::class,'regionDistribution']);
    Route::get('/charts/performance-trend', [\App\Http\Controllers\ChartsReportController::class,'performanceTrend']);
    Route::get('/charts/license-types', [\App\Http\Controllers\ChartsReportController::class,'licenseTypes']);
    Route::get('/recent-requests', [\App\Http\Controllers\ChartsReportController::class,'recentRequests']);
});
...
// التقارير (صفحة التقرير المخصص )
Route::prefix('custom-reports')->group(function () {
    Route::get('/', [\App\Http\Controllers\CustomReportController::class,'index']);
    Route::get('/meta', [\App\Http\Controllers\CustomReportController::class,'getMeta']);
    Route::post('/generate', [\App\Http\Controllers\CustomReportController::class,'generate']);

    Route::get('/export/csv', [\App\Http\Controllers\CustomReportController::class,'exportCsv']);
    Route::get('/export/excel', [\App\Http\Controllers\CustomReportController::class,'exportExcel']);
    Route::get('/export/pdf', [\App\Http\Controllers\CustomReportController::class,'exportPdf']);

    Route::post('/templates/save', [\App\Http\Controllers\CustomReportController::class,'saveTemplate']);
    Route::get('/templates/list', [\App\Http\Controllers\CustomReportController::class,'listTemplates']);
});
...
Route::middleware('auth:beneficiary')->group(function () {
    Route::get('/user-data/{id?}', [ProfileUserController::class, 'getUserData']);
    Route::post('/user-data/update', [ProfileUserController::class, 'update'])
          ->name('user.data.update');

    //تبويبات الحسابات المتصلة
    Route::get('/connected-accounts/{id?}',[ProfileUserController::class,'getConnectedAccounts']);
    Route::post('/connected-accounts/unlink',[ProfileUserController::class,'unlinkAccount']);
    Route::get('/activity-logs/{id?}',[ProfileUserController::class,'getActivityLogs']);

    Route::get('/security/data',[SecurityController::class, 'index']);
    Route::post('/security/toggle',[SecurityController::class, 'toggleSetting']);
    Route::post('/security/logout-device',[SecurityController::class, 'logoutDevice']);
    Route::post('/security/logout-all',[SecurityController::class, 'logoutAllDevices']);

});

Route::get('/fire-event', function () {
    event(new \App\Events\BeneficiaryAction(
        2, 'update', 'اختبار طبيعي', request()->ip(), substr(request()->userAgent() ?? '', 0, 190), 'صنعاء'
    ));
    return 'Event fired normally - check logs & DB';
});
